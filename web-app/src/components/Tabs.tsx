...
export function Tabs({
  ...
  const tabs: [string, string][] = role === 'admin' || role === 'auditor'
    ...
    : BASE_TABS;
  return (
    <div className="sticky top-0 z-20 bg-[#eef1f7]/90 backdrop-blur-md border-b border-slate-200/50 flex gap-6 px-6 -mx-6 pt-4 pb-0 mb-6 w-[calc(100%+3rem)] shadow-sm">
      {tabs.map(([id, label]) => (
        <button 
          key={id} 
